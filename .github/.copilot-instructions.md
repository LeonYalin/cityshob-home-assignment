...
#### Frontend Operations
**ALWAYS** change directory to `packages/frontend2` before running any frontend commands:
```bash
...
1. **Running frontend commands from root directory** - will fail because package.json scripts are in packages/frontend2
...
2. Terminal 2: Frontend server (`cd packages/frontend2 && npm start`)
